		...
		revObjVals.map((entry, i) => 
			<div key={i} className="card mb-3">
				...
				<ul className="list-group list-group-flush">
					...
					<li className='list-group-item'>Noncompliant Foods: {entry.nutrition_noncompliant.map((item, j) => <span key={j}>{item.item}{j === entry.nutrition_noncompliant.length - 1 ? '' : ', '} </span>)}</li>
